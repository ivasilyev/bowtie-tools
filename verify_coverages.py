...
def parse_args():
    ...
    starting_parser.add_argument("-r", "--refdata", default=None,
                                 help="(Optional) Refdata generated by 'cook_the_reference.py' script containing link to annotation file in 6th column")
...
def parse_namespace():
    ...
    if not namespace.annotation:
        if not namespace.refdata:
            raise ValueError("Please supply annotation or refdata file")
        else:
            try:
                namespace.annotation = file_to_list(namespace.refdata)[0].split("\t")[5]
            except IndexError:
                raise ValueError("Not enough columns in refdata file")
