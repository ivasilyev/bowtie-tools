...
def parse_args():
    ...
    starting_parser.add_argument("-i", "--input", required=True,
                                 help="Input list containing two tab-delimited columns for colorspace sequences and three for paired-end sequences: sample name and absolute path(s). May contain a header")
...
if __name__ == "__main__":
    ...
    for processed_sampledata_row in inputSampleDataList:
        if processed_sampledata_row.count('\t') < 1:
            ...
            sys.exit(2)
        if False in [os.path.isfile(input_path) for input_path in processed_sampledata_row.split('\t')[1:]] and sampleDataHeadersCount == 0:
            print("Header: " + processed_sampledata_row)
            inputSampleDataList.remove(processed_sampledata_row)
            sampleDataHeadersCount += 1
        else:
            [verify_path(input_path, 'file') for input_path in processed_sampledata_row.split('\t')[1:]]
